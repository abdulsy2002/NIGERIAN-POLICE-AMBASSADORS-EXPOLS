// src/app/gallery/3rd-agm/page.tsx
"use client";
import Link from 'next/link';
import { useState } from 'react';

export default function ThirdAGM() {
  const [lightbox, setLightbox] = useState<{ open: boolean; src: string; title: string }>({ open: false, src: '', title: '' });

  // Generate 318 images for 3rd AGM
  const images = Array.from({ length: 318 }, (_, i) => ({
    id: i + 1,
    title: `3rd AGM Photo ${i + 1}`,
    src: `/3rd agm/image (${i + 1}).jpg` // Change to .jpeg if needed
  }));
    ...
    <>
      <section className="hero-small" style={{background: "var(--primary)", color: 'white', padding: '60px 0', textAlign: 'center'}}>
        <div className="container">
          ...
          <h1 style={{fontSize: '2.5rem', marginTop: '10px'}}>3rd Annual General Meeting</h1>
          <p style={{opacity: 0.8}}>December 2025 • Kano State</p>
        </div>
      </section>
